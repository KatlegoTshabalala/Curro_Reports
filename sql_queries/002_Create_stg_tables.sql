...
IF NOT EXISTS (
    ...
    WHERE s.name = 'bronze_Curro' AND t.name = 'Grade10_Results'
)
BEGIN
    CREATE TABLE bronze_Curro.Grade10_Results(
...
INSERT INTO bronze_Curro.Grade10_Results
...
IF NOT EXISTS (
    SELECT 1 FROM sys.tables t JOIN sys.schemas s ON t.schema_id = s.schema_id
    WHERE s.name = 'bronze_Curro' AND t.name = 'Grade11_Results'
)
BEGIN
    CREATE TABLE bronze_Curro.Grade11_Results(
         [student_id] NVARCHAR(50)
        ,[student_name] NVARCHAR(200)
        ,[grade] NVARCHAR(10)
        ,[mathematics_mark] INT
...
INSERT INTO bronze_Curro.Grade11_Results
    ([student_id],[student_name],[grade],[mathematics_mark],[physical_science_mark],
     [life_sciences_mark],[english_home_language_mark],[life_orientation_mark],
     [information_technology_mark],[agricultural_science_mark],[total_mark],[average_mark])
SELECT
...
IF NOT EXISTS (
    SELECT 1 FROM sys.tables t JOIN sys.schemas s ON t.schema_id = s.schema_id
    WHERE s.name = 'bronze_Curro' AND t.name = 'Grade12_Results'
)
BEGIN
    CREATE TABLE bronze_Curro.Grade12_Results(
         [student_id] NVARCHAR(50)
        ,[student_name] NVARCHAR(200)
        ,[grade] NVARCHAR(10)
        ,[mathematics_mark] INT
...
INSERT INTO bronze_Curro.Grade12_Results
    ([student_id],[student_name],[grade],[mathematics_mark],[physical_science_mark],
     [life_sciences_mark],[english_home_language_mark],[life_orientation_mark],
     [information_technology_mark],[agricultural_science_mark],[total_mark],[average_mark])
SELECT
